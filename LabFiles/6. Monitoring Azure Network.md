...
5. Now try to establish the connection between **VM1-<inject key="DeploymentID" enableCopy="true"/>** and **VM3-<inject key="DeploymentID" enableCopy="true"/>** and check for the status.
     
    ![vm3](https://github.com/Divyasri199/AIW-Azure-Network-Solutions/blob/prod/media/check.png?raw=true)
          
      
6. You can see the status as **Unreachable**. The reason for getting the status as **Unreachable** is that you have the security group rule that blocks inbound traffic to **VM3-<inject key="DeploymentID" enableCopy="true"/>** on port 80.

     ![check](https://github.com/CloudLabsAI-Azure/AIW-Azure-Network-Solutions/blob/main/media/check1.png?raw=true)

7. If you want to establish the connection between **VM1-<inject key="DeploymentID" enableCopy="true"/>** and **VM3-<inject key="DeploymentID" enableCopy="true"/>**, navigate back to **VM3-<inject key="DeploymentID" enableCopy="true"/>** and enable the **Port_80** under **Inbound port rules** from **Networking** tab, then perform **Connection Troubleshoot**.
