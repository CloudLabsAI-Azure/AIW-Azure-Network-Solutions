...
The Web Portal is gaining attraction and Contoso is required to meet certain Security & Compliance guidelines before they can make it available to wider audience. Contoso wants to redesign and publish its application securely. 

You will help Contoso to meet the Security & Compliance guidelines by creating Azure firewall, Application Gateway (WAF) so that Firewall will be exposed to the end-users and traffic will be passed to the application gateway via azure firewall. 
...
8. On  the **Configuration** tab, select **Add a routing rule**.
...
14. Navigate to the overview page of the resource group and ensure that you will be able to see the deployed Application gateway **<inject key="AppgatewayName" enableCopy="false"/>**.
...
2. Now pass the value **?q=<script>** at the end of your **Firewall** IP and try browsing it. Notice that you can still access the webpage. In the next steps, you will learn how make it more secure with  web applicatoin firewall(WAF).
...
In the previous task, you have enhanced the security of your application with WAP policy. In this task you will configure outbound traffic to go via Azure firewall with the help of route table.
...
1. You can use the credentials mentioned below to login into virtual machine.
 ...
 1. Since the outbound access to the virtual machine is not allowed, you should be blocked by the firewall.
...
1. Now, navigate back to the remote desktop and refresh the tab, you will be able to access the Bing page. Now open Microsoft Edge and search for **What is my IP Address**. You will be able to view the IP Address of the Azure Firewall. This will confirm that the outbound traffic is moving from Firewall.
