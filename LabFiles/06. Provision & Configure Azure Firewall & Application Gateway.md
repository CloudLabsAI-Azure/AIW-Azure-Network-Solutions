...
You will help Contoso to meet the Security & Compliance guidelines by creating an Azure firewall, Application Gateway (WAF) so that the Firewall will be an internet facing resource and all traffic will be passed to the application gateway via azure firewall. 
...
1. Navigate to the overview page of the resource group and ensure that you can see the deployed Application gateway **<inject key="AppgatewayName" enableCopy="false"/>**.
...
In the previous task, you have enhanced the security of your application with the WAF policy. In this task, you will configure user defined route and outbound traffic to go via Azure firewall.
