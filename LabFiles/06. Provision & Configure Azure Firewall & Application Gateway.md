...
You will help Contoso to meet the Security & Compliance guidelines by creating an Azure firewall, Application Gateway (WAF) so that Firewall will be exposed to the end-users and traffic will be passed to the application gateway via azure firewall. 
...
2. Now pass the value **?q=<script>** at the end of your **Firewall** IP and try browsing it. Notice that you can still access the webpage. In the next steps, you will learn how to make it more secure with web application firewall(WAF).
