...
18. On **Add a load balancing rule** page enter the below details:
     ...
     - Frontend IP address : **<inject key="Ipname" enableCopy="true"/>** from drop down list.

     - Port : **80**

     - Backend Port : **80**

     - Backend Pool : **<inject key="Backendpoolname" enableCopy="true"/>** from drop down list.
...
4. To test for VM failover, navigate back to resource group **<inject key="Resource Group" enableCopy="true"/> ** and select **<inject key="VM1Name" enableCopy="true"/>**.
...
5. Navigate back to the browser where you accessed the public IP address and refresh the page, now observe the request only served by **<inject key="VM2Name" enableCopy="true"/>**.
