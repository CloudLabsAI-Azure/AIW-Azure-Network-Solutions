...
Contoso now has their network ready, they want to host their Contoso Insurance application on a server that provides more control and flexibility over the computing environment. Contoso wants  to provision virtual machine and host contoso insurance application on it, so that internal users can test the application.
