...
Contoso Insurance application has been tested and validate by internal users. Now, Contoso wants their application to be available for beta users for testing. Contoso wants to  distribute their application traffic across servers and want its application to be highly available. They also want to control the system failures with zero downtime. 
